/*
From: https://playground.babylonjs.com/#P1RZV0
*/

import * as BABYLON from 'babylonjs'
import 'babylonjs-loaders'

const _createScene = async function (engine) {
  ...
  const loadTexturesAsync = function () {
    const textures = []
  ...
  const whenAllReady = function (textures, resolve) {
  ...
  const retrieveTexture = function (meshMat, channel, textures) {
    let texture
  ...
  async function createMaterials() {
    ...
    bottleParameters.baseColor.texture = bottleParameters.labelBaseColor.texture =
      bottleTex.baseColor
    ...
    bottleParameters.thickness.texture = bottleParameters.labelThickness.texture =
      bottleTex.thickness
    bottleParameters.translucency.texture = bottleParameters.labelTranslucency.texture =
      bottleTex.translucency
...
async function createScene(engine) {
  const scene = await _createScene(engine)
  return scene
}
